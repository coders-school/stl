...
double ArithmeticAverage(const std::vector<int> & firstVec, const std::vector<int> & secVec) {
    double init = 0.0;
    return std::inner_product(firstVec.cbegin(), firstVec.cend(), secVec.cbegin(), init,
...
double Distance(const std::vector<int> & pointA, const std::vector<int> & pointB) {
    double init = 0.0;
    return std::sqrt(std::inner_product(pointA.cbegin(), pointA.cend(), pointB.cbegin(), init,
