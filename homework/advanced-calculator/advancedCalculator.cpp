...
std::map<const char, std::function<double(const double, const double)>>
    operations{{'+', std::plus<double>{}},
               ...
               {'!',
                [](const auto number, const auto temp = 1) {
                    return number > 0 ? std::tgamma(number + 1)
                                      : std::tgamma(-number + 1) * -1;
                }},
               {'^', [](const auto base,
                        const auto exp) { return std::pow(base, exp); }},
               {'$', [](const auto value, const auto n) {
                    return std::pow(value, 1 / n);
                }}};
...
bool is_bad_character(const std::string &expression) {
    return !std::all_of(
        expression.cbegin(), expression.cend(), [](const auto ch) {
            return std::isdigit(ch) ||
                   std::find(valid_operators.cbegin(), valid_operators.cend(),
                             ch) != valid_operators.cend() ||
...
bool is_module_of_non_integer_value(const tuple_type &expression) {
    auto [lhs, op, rhs] = expression;
    return op.value() == '%' && (std::floor(lhs.value()) != lhs.value() ||
...
tuple_type get_values(const std::string &input) {
    ...
    if (operation == '!')
        return tuple_type{lhs, operation, std::optional<double>{}};
    double rhs{};
    input_stream >> rhs;
    if (!input_stream)
        return tuple_type{lhs, operation, std::optional<double>{}};
...
ErrorCode check_errors(const tuple_type &expression) {
    using pair_type =
        std::pair<std::function<bool(const tuple_type &)>, ErrorCode>;
    static std::array<pair_type, 5> validation_functions{
        ...
        pair_type{is_module_of_non_integer_value,
                  ErrorCode::ModuleOfNonIntegerValue}};
    auto error_pair_it = std::find_if(
        validation_functions.cbegin(), validation_functions.cend(),
        [&expression](const auto &pair) { return pair.first(expression); });
    return error_pair_it == validation_functions.cend() ? ErrorCode::OK
                                                        : error_pair_it->second;
}
