...
void printMap(std::array<std::array<uint8_t, 32>, 32>& m) {
...
std::vector<std::pair<uint8_t, uint8_t>> compressGrayscale(std::array<std::array<uint8_t, width>, height>& raw_bitmap) {
    std::vector<std::pair<uint8_t, uint8_t>> compress_bitmap = {};

    uint8_t number_of_dupplicates = 1;
    bool new_row = true;
    uint8_t actual_color = 0;
    for (const auto& row : raw_bitmap) {
        for (const auto& el : row) {
            if (new_row) {
                actual_color = el;
                number_of_dupplicates = 1;
                new_row = false;
            } else if (actual_color == el) {
                number_of_dupplicates++;
            } else {
                compress_bitmap.push_back(std::pair<uint8_t, uint8_t>(actual_color, number_of_dupplicates));
                actual_color = el;
                number_of_dupplicates = 1;
            }
        }
...
std::array<std::array<uint8_t, width>, height> decompressGrayscale(std::vector<std::pair<uint8_t, uint8_t>>& compress_bitmap) {
    std::array<std::array<uint8_t, width>, height> raw_bitmap = {};

    std::size_t col_actual = 0;
    std::size_t row_actual = 0;
    for (const auto& el : compress_bitmap) {
        for (std::size_t i = 0; i < el.second; ++i) {
            raw_bitmap.at(row_actual).at(col_actual) = el.first;
            col_actual++;
            if (col_actual >= width) {
                col_actual = 0;
                row_actual++;
